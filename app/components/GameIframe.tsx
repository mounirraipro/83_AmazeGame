...
export default function GameIframe({ src, title, className, style, scrolling }: GameIframeProps) {
  ...
  const handleShare = async () => {
    try {
      if (navigator.share) {
        await navigator.share({ title: 'Amaze Game', text: 'Play this free puzzle game!', url: window.location.href });
      } else {
  ...
  return (
    ...
    >
      ...
      <div className="game-iframe-area">
        ...
        {!gameStarted && !inFull && (
          <div className="game-play-overlay" onClick={handleStartGame} role="button" aria-label="Click to play Amaze Game">
